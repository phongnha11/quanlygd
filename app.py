...
# --- KẾT NỐI GOOGLE SHEETS (CACHED - AN TOÀN) ---
# Cache connection object, NOT data. Dữ liệu luôn được fetch mới khi cần.
@st.cache_resource
...
# Cache việc mở file spreadsheet để tránh tìm kiếm file nhiều lần
...
def get_spreadsheet(_client):
    try:
        ...
        try:
            ...
            st.error(f"⚠️ Không tìm thấy file Google Sheet tên là '{SPREADSHEET_NAME}'.")
            st.info("👉 Vui lòng đảm bảo bạn đã tạo file này và chia sẻ quyền Editor cho email Service Account.")
            st.stop()
    except Exception as e:
        st.error(f"❌ Lỗi mở file Sheet: {e}")
        return None

client = get_gsheet_client()
sh = get_spreadsheet(client) if client else None
...
def get_worksheet(sheet_name):
    ...
    try:
        ...
        st.error(f"⚠️ Lỗi truy cập sheet '{sheet_name}': {e}")
        st.stop()
...
# KHÔNG DÙNG @st.cache_data ở đây để đảm bảo dữ liệu luôn tươi mới
...
def main():
    if not client or not sh:
        st.warning("Đang kết nối cơ sở dữ liệu... Nếu thấy lâu, hãy tải lại trang.")
        st.stop()
        ...
        with tab1:
            with st.form("config_form"):
                ...
                if st.form_submit_button("Lưu Cấu hình"):
                    ...
                    # KHÔNG CẦN clear cache vì get_data không cache
                    time.sleep(1)
                    st.rerun()
